...
Camera::Camera(glm::vec3 t_pos) 
	: position(t_pos), world_up(glm::vec3(0.f, 1.f, 0.f)),
		yaw(-90.f), pitch(0.f), move_speed(2.5f), zoom(45.f), forward(glm::vec3(0.f, 0.f, -1.f)) {
	update_vectors();
}
...
/// <summary>
/// Temp. | Change camera zoom.
/// </summary>
/// <param name="t_dy"></param>
void Camera::update_zoom(double t_dy) {
	//Clamp zoom.
	if(zoom >= 1.f && zoom <= 45.f) zoom -= t_dy;
	else if(zoom < 1.f) zoom = 1.f;
	else zoom = 45.f;
}
