...
GPUInfo getGPUInfo() {
    ...
    std::string lspci=INTERNAL::execute("lspci | grep -i vga");
    if (!lspci.empty()) {
    ...
    std::string glxinfo=INTERNAL::execute("glxinfo -B | grep -i 'Video memory'");
    if (!glxinfo.empty()) {
...
MotherboardInfo getMotherboardInfo() {
    ...
    std::string dmidecode=INTERNAL::execute("sudo dmidecode -t baseboard");
    if (!dmidecode.empty()) {
