...
namespace Firesteel {
    namespace FBX {
        static Texture loadMaterialTexture(const Model* tModel, const char* tTexPath, const TextureType& tType) {
            if (!tTexPath) return {};
        ...
        static Mesh processMesh(Model* tBaseModel, const ufbx_mesh* tMesh
#ifdef FS_PRINT_DEBUG_MSGS
        ...
        static void processNodes(Model* tBaseModel, const ufbx_scene* tModel, const ufbx_node* tNode, std::shared_ptr<Node> tParent = nullptr) {
        ...
        static void tryAddTexture(const ufbx_material_map* tTex, Model* tModel, Material* tMat, const TextureType& tType) {
