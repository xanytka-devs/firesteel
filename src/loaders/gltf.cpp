...
#include <model_loaders/tiny_gltf.h>

namespace Firesteel {
    namespace GLTF {
        static Texture loadMaterialTexture(const Model* tModel, const std::string& texPath, const TextureType& type) {
        ...
        static std::string getTexPath(const tinygltf::Model* tModel, const int tId) {
            if (tId >= 0) return tModel->images[tModel->textures[tId].source].uri;
            return "";
        }

        static bool hasAttribute(const tinygltf::Primitive* tPrimitive, const char* tAttrib) {
            return tPrimitive->attributes.find(tAttrib) != tPrimitive->attributes.end();
        }

        static void processNodes(Model* tBaseModel, const tinygltf::Model* tModel, int tIndex, std::shared_ptr<Node> tParent = nullptr) {
        ...
        static Mesh processPrimitive(Model* tBaseModel, const tinygltf::Model* tModel, const tinygltf::Primitive* tPrimitive, const std::string tPath
#ifdef FS_PRINT_DEBUG_MSGS
