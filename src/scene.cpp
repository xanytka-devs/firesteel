...
namespace Firesteel {
#ifndef FS_NO_JSON
    bool Scene::load(const std::string& tPath) {
    ...
    void Scene::save(const std::string& tPath, const bool& tPrettyPrint) const {
    	nlohmann::json scene;
    ...
    std::shared_ptr<Entity> Scene::add(std::shared_ptr<Entity>& tEntity) {
    	entities.push_back(tEntity);
    	return tEntity;
    }
    std::shared_ptr<Entity> Scene::add(Entity& tEntity) {
    	auto ent = std::make_shared<Entity>(tEntity);
    	entities.push_back(ent);
    	return ent;
    }
    std::shared_ptr<Entity> Scene::create() {
    	auto ent = std::make_shared<Entity>();
    	entities.push_back(ent);
    	return ent;
    }
    bool Scene::removeAt(const ulong& tId) {
